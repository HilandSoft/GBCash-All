namespace YingNet.WeiXing.Business.CommonLogic
{
    ...
    public class PayDaySchedule
    {
        ...
        public static bool CalculatePayLoan(Page currentPage, float numIncomeOrBenefit, float numLoanAmount, int numInstallmentCount, bool isFirstLoan, ref decimal[] payAmounts, out string errorString)
        {
            float num = 0f;
            float num2 = 0f;
            float num3 = 0f;
            ...
            decimal payAmountPerTime = (decimal)num12 / ((decimal)numInstallmentCount);

            payAmountPerTime= decimal.Round(payAmountPerTime,2);

            //将总金额分成多期之后，每期由于四舍五入的原因，
            //将各期的金额再进行加和之后，会跟原来的总额略有不同
            //因此使用本方法，将差额在最后一个分期中进行修正
            for (int i = 0; i < numInstallmentCount; i++)
            {
                if (i == numInstallmentCount - 1)
                {
                    payAmounts[i] = (decimal)num12 - payAmountPerTime * i;
                }
                else
                {
                    payAmounts[i] = payAmountPerTime;
                }
            }
            errorString = string.Empty;
            return true;
